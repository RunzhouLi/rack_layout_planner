...
function constructPR(scene, configurator, clonedObjects, sceneObject, loop) {
    ...
    dynamicText = new dynText(scene, measurement, fontSize, totalWidth, 0.01, new Vector3(0-(standWidth/200), 0.01, totalWidth / 2 + 0.8), 0, -MathUtils.degToRad(90), "#556879", 1, null, " cm", loop, 0);
    dynamicText.addText();
    ...
    rackGroups.forEach(group => {
        ...
        if (!depthMeasurements.has(key)) {
            ...
            const dynamicText2 = new dynText(scene, measurement, fontSize, rackDepth, 0.01, new Vector3(-totalWidth / 2 - 0.8, 0.01, (group.offsetZ || 0) - depthOffset), -MathUtils.degToRad(90), 0, "#556879", 1, null, " cm", loop, 0);
    ...
    rackGroups.forEach(group => {
        for (let rackIndex = 0; rackIndex < group.units.length; rackIndex++) {
            if ((configurator.startPostIncrease && rackIndex == 0) || (configurator.endPostIncrease && rackIndex == group.units.length-1 && !configurator.addOnRack)) {
                increaseOffset = 50;
            }
            else increaseOffset = 0;

            heightArray.push(group.units[rackIndex].height + increaseOffset);
        }
    });
    ...
    dynamicText3 = new dynText(scene, measurement, fontSize, measurement / 200 * 2, 0.01, new Vector3(totalWidth / 2 + 0.8, measurement/200 + 0.02, 0), 0, 0, "#556879", 1, null, " cm", loop, 0);
