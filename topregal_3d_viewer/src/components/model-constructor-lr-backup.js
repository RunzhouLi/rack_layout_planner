...
function constructLR(scene, configurator, clonedObjects, sceneObject, loop) {
    ...
    rackGroups.forEach((group, i) => {
        ...
        let groupWidthOffset = 0;        // Expand units with count property for rendering
        const expandedUnits = [];
        group.units.forEach(unit => {
            const count = unit.count || 1;
            for (let i = 0; i < count; i++) {
                const expandedUnit = { ...unit };
                delete expandedUnit.count; // Remove count property from expanded unit
                expandedUnits.push(expandedUnit);
            }
        });
        
        expandedUnits.forEach((unit, standIndex) => {
            ...
            groupWidthOffset += (unit.width + standWidth) / 100;            // right stand
            if (standIndex === unit.shelves.length - 1 || (unit.height >= (expandedUnits[standIndex+1]?.height || 0))) {
